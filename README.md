...
python3 tests/test_store.py     # the persisted state and its schema version
```

All four suites run in CI on every push and pull request, next to a
`home-assistant/builder` test build for all supported architectures. The green
badge means exactly those four passed: the git engine, the translation
dictionaries, the quiet-period timer and the state file. The FastAPI
endpoints, the panel and the GitHub client have no automated coverage — they
are still checked by hand.
